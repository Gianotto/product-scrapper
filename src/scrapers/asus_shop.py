...
class ASUSShopScraper(BaseScraper):
    ...
    name = "asus_shop"
    _referer = "https://shop.asus.com/"
    _debug_html_name = "debug_asus_shop.html"
    ...
    def search(self, product: Product, search_url: str) -> list[PriceCheck]:
        ...
        try:
            ...
            logger.warning("ASUSShop fetch failed for {url}: {exc}", url=search_url, exc=exc)
            return []
        ...
        if not cards:
            logger.warning(
                "No product cards found on ASUS Shop results page for {url}", url=search_url
            )
        ...
        for card in cards:
            try:
                ...
                logger.debug("Skipping malformed ASUS Shop card: {exc}", exc=exc)
                continue
        ...
        if not filtered:
            logger.warning(
                "ASUSShop: zero results after filtering for SKU={sku}", sku=product.sku
            )
            self._dump_debug_html(html)
            return []
        ...
        logger.info(
            "ASUSShop: found {n} result(s) for SKU={sku}", n=len(checks), sku=product.sku
        )
        return checks
